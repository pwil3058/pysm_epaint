...
GENERAL_WORDS_FILE_PATH = os.sep.join([CONFIG_DIR_PATH, "general_words"])

def read_general_words():
    general_words = []
    if os.path.isfile(GENERAL_WORDS_FILE_PATH):
        for line in open(GENERAL_WORDS_FILE_PATH, 'r').readlines():
            paint_word = line.strip()
            if len(paint_word) == 0:
                continue
...
def append_general_words(general_words):
    fobj = open(GENERAL_WORDS_FILE_PATH, 'a')
