    ...
    def rotated(cls, rgb, delta_hue_angle):
        ...
        if delta_hue_angle > 0:
            if delta_hue_angle > mathx.PI_120:
                ...
                return array.array(cls.ARRAY_TYPECODE, (f(2, 1), f(0, 2), f(1, 0)))
            else:
                k1, k2 = calc_ks(delta_hue_angle)
                return array.array(cls.ARRAY_TYPECODE, (f(0, 2), f(1, 0), f(2, 1)))
        elif delta_hue_angle < 0:
            if delta_hue_angle < -mathx.PI_120:
                k1, k2 = calc_ks(abs(delta_hue_angle) - mathx.PI_120)
                return array.array(cls.ARRAY_TYPECODE, (f(1, 2), f(2, 0), f(0, 1)))
            else:
                k1, k2 = calc_ks(abs(delta_hue_angle))
                return array.array(cls.ARRAY_TYPECODE, (f(0, 1), f(1, 2), f(2, 0)))
        else:
            return rgb
...
class HueNG(collections.namedtuple("Hue", ["io", "other", "angle", "chroma_correction"])):
    ...
    def rgb(self):
        if math.isnan(self.angle):
            return array.array(self.ARRAY_TYPECODE, (self.ONE, self.ONE, self.ONE))
        result = array.array(self.ARRAY_TYPECODE, [self.ZERO, self.ZERO, self.ZERO])
    ...
    def rgb_with_total(self, req_total):
        ...
        if math.isnan(self.angle):
            ...
            return array.array(self.ARRAY_TYPECODE, (val, val, val))
    ...
    def get_xy_for_chroma(self, chroma):
        ...
        return Cartesian(hypot * math.cos(self.angle), hypot * math.sin(self.angle))
...
class Cartesian(collections.namedtuple("Cartesian", ["x", "y"])):
    ...
    def from_rgb(cls, rgb):
        ...
        >>> Cartesian.from_rgb((100, 0, 0))
        Cartesian(x=Fraction(100, 1), y=Fraction(0, 1))
        """
    ...
    def get_hypot(self):
        ...
        >>> Cartesian.from_rgb((100, 0, 0)).get_hypot()
        Fraction(100, 1)
        >>> round(Cartesian.from_rgb((100, 100, 100)).get_hypot())
        0.0
        >>> round(Cartesian.from_rgb((0, 100, 0)).get_hypot())
        100.0
        >>> round(Cartesian.from_rgb((0, 0, 100)).get_hypot())
        100.0
        >>> round(Cartesian.from_rgb((0, 100, 100)).get_hypot())
        100.0
        >>> round(Cartesian.from_rgb((0, 100, 50)).get_hypot())
        87.0
        """
        return math.hypot(self.x, self.y)
    def get_frgb(self):
        # get the RGB values for this Cartesian point as floating point numbers
...
class RGBManipulator(object):
    ...
    def _set_from_chroma(self, new_chroma):
        ...
        new_base_rgb = Cartesian(*[c * ratio for c in self.xy]).get_frgb().converted_to(RGBPN)
